...
3. O Auth System cria a Checkout Session usando o `STRIPE_PRICE_ID` configurado
   no Worker e devolve somente a URL hospedada pela Stripe.
4. A Stripe envia `checkout.session.completed` e eventos de assinatura a
   `POST /v1/stripe/webhook`.
5. O Worker valida a assinatura, confirma a assinatura na API Stripe e grava o
   estado em `stripe_subscriptions`.
